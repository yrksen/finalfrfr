...
import { Search, Plus, X, Filter, Star, RefreshCw } from 'lucide-react';
...
function HomePage({ currentUser, setCurrentUser, isDarkMode, setIsDarkMode }: { currentUser: any, setCurrentUser: any, isDarkMode: boolean, setIsDarkMode: any }) {
  ...
  const handleUpdateCastDirector = async () => {
    ...
    try {
      ...
      // Use movies that are already loaded on the page
      const allMovies = movies;

      // Filter movies that need cast/director updates
      const moviesToUpdate = allMovies.filter((m: Movie) =>
        m.imdbId && (!m.cast || !m.director || m.cast.length === 0 || m.director === "N/A")
      );

      console.log(`Found ${moviesToUpdate.length} movies that need cast/director updates`);
      console.log('Movies to update:', moviesToUpdate.map(m => m.title));
      setUpdateProgress(`Found ${moviesToUpdate.length} movies to update...`);

      if (moviesToUpdate.length === 0) {
        setUpdateProgress(`✅ All movies already have cast and director information!`);
        setTimeout(() => {
          setUpdateProgress('');
      ...
      // Process in batches of 3 to be more cautious
      const batchSize = 3;
      for (let i = 0; i < moviesToUpdate.length; i += batchSize) {
        const batch = moviesToUpdate.slice(i, i + batchSize);
        const currentProgress = i + totalUpdated;
        setUpdateProgress(`Updating... (${currentProgress}/${moviesToUpdate.length} movies) - Processing ${batch[0].title}`);
        console.log(`\n=== Processing batch ${Math.floor(i / batchSize) + 1} ===`);

        // Process batch
        for (const movie of batch) {
          try {
            console.log(`\n🎬 Processing: ${movie.title} (ID: ${movie.id}, IMDB: ${movie.imdbId})`);

            // Fetch from OMDB API with timeout
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 10000);

            try {
              const omdbResponse = await fetch(`https://www.omdbapi.com/?i=${movie.imdbId}&apikey=f9062e1`, {
                signal: controller.signal
              });
              clearTimeout(timeoutId);

              if (!omdbResponse.ok) {
                console.error(`OMDB API failed for ${movie.title}: ${omdbResponse.status}`);
                totalErrors++;
                continue;
              }

              const omdbData = await omdbResponse.json();
              console.log(`OMDB response for ${movie.title}:`, omdbData.Response);

              if (omdbData.Response === "True") {
                const updates: any = {};
                let hasUpdates = false;

                if ((!movie.cast || movie.cast.length === 0) && omdbData.Actors && omdbData.Actors !== "N/A") {
                  updates.cast = omdbData.Actors.split(', ').slice(0, 5);
                  hasUpdates = true;
                  console.log(`  Adding cast: ${updates.cast.join(', ')}`);
                }

                if ((!movie.director || movie.director === "N/A") && omdbData.Director && omdbData.Director !== "N/A") {
                  updates.director = omdbData.Director;
                  hasUpdates = true;
                  console.log(`  Adding director: ${updates.director}`);
                }
                ...
                if (hasUpdates) {
                  ...
                  const updateResponse = await fetch(`${API_BASE_URL}/movies/${movie.id}`, {
                    method: 'PATCH',
                    headers: {
                      'Authorization': `Bearer ${publicAnonKey}`,
                      'Content-Type': 'application/json',
                  ...
                  if (updateResponse.ok) {
                    ...
                    console.error(`  ❌ Failed to update ${movie.title}: ${updateResponse.status} - ${errorText}`);
                    totalErrors++;
                  }
                } else {
                  console.log(`  ⚠️ No updates needed (already has cast/director)`);
                  totalSkipped++;
                }
              } else {
                console.log(`  ⚠️ OMDB returned False for ${movie.title}: ${omdbData.Error}`);
                totalSkipped++;
              }
            } catch (fetchError) {
              clearTimeout(timeoutId);
              if (fetchError.name === 'AbortError') {
                console.error(`  ⏱️ Timeout fetching OMDB data for ${movie.title}`);
              } else {
                console.error(`  ❌ Fetch error for ${movie.title}:`, fetchError);
              }
              totalErrors++;
            }
            ...
            console.error(`  ❌ Unexpected error updating ${movie.title}:`, error);
            totalErrors++;
          }
        }
      ...
      setUpdateProgress(`✅ Complete! Updated: ${totalUpdated} | Skipped: ${totalSkipped} | Errors: ${totalErrors} | All done!`);
      console.log(`
========================================
UPDATE COMPLETE!
========================================
Updated: ${totalUpdated} movies
Skipped: ${totalSkipped} movies (already had data)
Errors: ${totalErrors} movies (failed to update)
Total processed: ${moviesToUpdate.length} movies
========================================
      `);
      setTimeout(() => {
        loadMovies();
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'dark' : ''}`}>
      ...
      >
        <div className="flex items-center justify-between gap-4 px-6">
          ...
          <div className="absolute hidden md:flex items-center gap-4" style={{ left: 'calc(100vw / 8)', width: 'auto' }}>
                  ...
                  {displayMovies.slice(0, 8).map((movie) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {movie.genre && (
                          <div className={`text-xs truncate mt-0.5 ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                            {movie.genre}
                          </div>
                        )}
            ...
            {/* Update Cast & Director button */}
            <button
              onClick={handleUpdateCastDirector}
              disabled={isUpdatingCastDirector}
              className={`flex items-center gap-1.5 text-sm font-medium transition-opacity tracking-tight whitespace-nowrap ${
                isUpdatingCastDirector
          ...
          <div className="fixed inset-0 z-50 md:hidden">
            ...
            <div className={`absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] shadow-xl overflow-y-auto ${isDarkMode ? 'bg-gray-800' : 'bg-background'}`}>
              <div className={`p-4 border-b flex items-center justify-between ${isDarkMode ? 'border-gray-700' : ''}`}>
                <h2 className={`text-sm font-bold tracking-tight ${isDarkMode ? 'text-white' : ''}`}>Filters</h2>
                <button
                  onClick={() => setIsMobileFilterOpen(false)}
        ...
        <main className={`flex-1 p-4 md:p-8 ${isDarkMode ? 'text-white' : ''}`}>
          ...
          <div className="hidden md:flex mb-6 items-center justify-between">
            <div className="flex items-center gap-3">
              <p className={`text-sm leading-normal font-bold italic ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          ...
          {(selectedGenres.length > 0 || selectedYears.length > 0 || searchQuery !== "" || selectedTags.length > 0 || imdbRatingRange[0] !== 0 || runtimeFilter !== 'all') && (
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selectedYears.map((year) => (
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${isDarkMode ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
                  >
                    {year}
                    <X className="size-3.5" />
                  </button>
                ...
                {imdbRatingRange[0] !== 0 && (
                  <button
                    onClick={() => setImdbRatingRange([0, 10])}
                  ...
                  >
                    IMDb: {imdbRatingRange[0].toFixed(1)} - {imdbRatingRange[1].toFixed(1)}
                    <X className="size-3.5" />
                  </button>
                )}
                {runtimeFilter !== 'all' && (
                  ...
                  >
                    Runtime: {runtimeFilter === 'short' ? '≤90 min' : runtimeFilter === 'medium' ? '90-150 min' : runtimeFilter === 'long' ? '≥150 min' : runtimeFilter === 'oneSeason' ? '1 Season' : 'Multi-Season'}
                    <X className="size-3.5" />
                  </button>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selectedYears.map((year) => (
                  <button
                    key={year}
                    onClick={() => handleYearChange(year, false)}
                    className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium transition-colors ${isDarkMode ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
                  >
                    {year}
                    <X className="size-3" />
                  </button>
      ...
      {/* Chat Assistant */}
      <ChatAssistant movies={currentView === 'towatch' ? toWatchMovies : movies} isDarkMode={isDarkMode} />
        </>
      )}
    </div>
  );
